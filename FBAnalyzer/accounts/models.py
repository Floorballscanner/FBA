...
class Shot(models.Model):
    ...
    shooter = models.CharField(max_length=50, blank=True)
    passer = models.CharField(max_length=50, blank=True)
    ...
    def __str__(self):
        return f'Game: {self.game} Time: {self.time}, Shot type: {self.type}, Shot result: {self.result}, xG: {self.xG}'
...
class License(models.Model):
    ...
    FLIIGA_TRIAL_DURATION = timedelta(days=7)
    ...
    def save(self, *args, **kwargs):
        if self.starts_at is None:
            ...
            elif self.tier == 'fliiga_trial':
                duration = self.FLIIGA_TRIAL_DURATION
            else:
